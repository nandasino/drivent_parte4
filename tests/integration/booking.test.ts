...
describe("POST /booking", () => {
  ...
  describe("when token is valid", () => {
    ...
    it("should respond with status 400 whith a invalid roomID", async () => {
      ...
      const response = await server.post("/booking")
      .set("Authorization", `Bearer ${token}`).send({"roomId": 0});
    ...
    it("should respond with status 404 if room does not exist", async () => {
      ...
      const response = await server.post("/booking")
      .set("Authorization", `Bearer ${token}`).send({"roomId": roomIdNotCreated});
...
describe("PUT /booking", () => {
  ...
  describe("when token is valid", () => {
    ...
    it("should respond with status 404 if room does not exist", async () => {
      ...
      const response = await server.put(`/booking/${booking.id}`)
      .set("Authorization", `Bearer ${token}`).send({"roomId": newRoomNotExist});
    ...
    it("should respond with status 403 when booking does not exist", async () => {
      ...
      const newRoom = await createRoomWithHotelId(hotel.id);
      const response = await server.put(`/booking/${booking.id}`).set("Authorization", `Bearer ${token}`).set("Authorization", `Bearer ${token}`)
